...
func (s *Server) adminRecipeSchema(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, 200, map[string]any{
		...
		"rules": map[string]any{
			...
			"hidden":      "true — спрятать (нет в планировщике, каталоге, поиске и sitemap) до вычитки; по умолчанию рецепт виден сразу. Открыть скрытый: POST /api/admin/recipes/{id}/publish",
