...
### Rounds 27–31: catalog filters, collection page, header switchers, comments, prep mode, offers
- **Catalog with side filters (`.catmain`, `.filters`, `.prange`):** on ≥900px the filter panel is sticky at `top: 12px` with its own thin scrollbar; on phones it opens full-screen (`.filters.is-open`, `sheet-up`) with a sticky «Готово». The price filter is a two-handle range (`.prange`: two overlaid `input[type=range]`, a blue track segment between the handles, «от … до …» numbers in `.num`) plus quick chips «до 100 ₽ / до 200 ₽». Filter changes swap only the sidebar and the main column (`ssr.js`), no page reload; while loading the grid keeps its height and shows a spinner, never a blur.
- **Country and currency (`.theme-btn--country`, `.countrydlg`):** a header button `flag 18×13 + currency symbol 13/600` next to the language button on SSR pages; opens the same `.bsheet` list as languages (`.langlist`), rows `flag · country name + small «RUB · ₽»`, current on blue-tint. Choosing sets `?country=XX` and the cookie. In the app the country lives in the quiz's first step, so the app header has no switcher.
- **Collection page, rich (`.colhead--rich`, `.colfacts`, `.colgroup`, `.colhow`, `.faq`):** hero 5/7 with cover, eyebrow-free title 28/700, lead, a facts row `12 рецептов · 92–343 ₽ за порцию · 15–20 мин · 424–661 ккал` (`b.num` 16/700 over 12px label), two buttons (primary «Собрать неделю из коллекции», soft «Поделиться»), then one or two intro paragraphs 16/1.5 inside the hero body. Recipes grouped by course (`.colgroup`, `h2 + .catsection__n`) when salads/desserts dominate, otherwise by meal. Below: a two-column `.colhow` card (bg-elevated, xl radius): «Как пользоваться» paragraphs + primary button / «Вопросы» as `<details class="faq">` rows with a rotating plus. «Другие подборки» as `.colmini` chips in a wrapping row. All texts hand-written (plain-prose), no currency amounts inside prose.
- **Recipe notes («Советы», `.notes__list`):** a `<dl>` with 13/600 tertiary labels («Почему так», «Чем заменить», «Где ошибаются», «Хранить и разогреть», «С чем подать») in the left column and 15/1.45 text on the right; single column under 640px.
- **Sides (`.dish__side`):** under a main dish, a full-width line `«Гарнир» tag (11/600 uppercase, blue on blue-tint, radius 6) · side title · kcal .num 12 tertiary · 24px swap button`. On the recipe page, sections «Подойдёт гарнир» / «Гарнир к блюдам».
- **Ingredient thumbnails (`.pic__btn`, `.pic__thumb`, `.pic__float`):** the ingredient row ends with a 24px button holding a 22px rounded photo (8px gap from the name); hover / tap opens a 160px `popover="manual"` image in the top layer (so it shows above `<dialog>` and never widens the sheet). On touch it stays until scroll or a tap elsewhere; scroll within 400ms of opening is ignored (focus-induced).
- **Comment editor (`.mdedit`, `.mdbar`, `.mdlink`):** a `contenteditable` field styled as `.form-control` (min 84px) with placeholder via `data-ph`; toolbar buttons show `aria-pressed` blue-tint for the active format; bold/italic/strike/list/quote render immediately, pasted Markdown is converted on the fly; the link tool opens an inline row `url input + «Добавить» + «Отмена»` with a red 13px error line instead of the browser prompt. Stored as Markdown. No «поддерживается Markdown» hint anywhere.
- **Leaving warning (`.leave`):** clicking an external link inside a comment opens a `.bsheet`: «Вы уходите с Рациона», lead with the host in `b`, the full URL in 13px tertiary mono, two buttons «Перейти» (primary, opens in a new tab) and «Отмена». Partner links are exempt.
- **Action bars:** the plan keeps the fixed bottom `.actionbar`; on ≤480px its tools (print, family, repeat, share, assistant) collapse into a `⋯` button (`.actionbar__more`) that opens `.actionmenu` (bg-elevated, lg radius, shadow lg, rows `icon 20px + 15px label`), so «В магазин» keeps its text. The quiz's «Дальше» is `.actionbar--flow`: static under the step, no backdrop, no top rule.
- **Prep mode (`.prep`, «Заготовки»):** a receipt section before the days, styled like `.day`: head `Заготовки <small>раз в неделю</small>` with the sum `14 блюд · 4 ч 3 мин у плиты` and a chevron to collapse. Each prep session is `.prep__session`: `b Воскресенье перед неделей · small date · «сначала то, что готовится дольше»` and a table of rows `58px .num minutes tertiary · dish title 15/500 (side after «+» in secondary, both clickable) · day pills (11.5/600 uppercase on fill-4, «×4» when cooked for several days) · 20px store icon (Refrigerator quaternary or Snowflake blue)`. Rows sorted by time descending. On phones day pills drop under the title. In `.dish__why` a 12px tag: `Refrigerator «заготовка · разогреть»`, `Snowflake «из морозилки · разморозить с вечера»` (blue), `Flame «готовим свежим»` (green).
- **Quiz option rows for modes (`.option.option--compact` + `.option__val`):** three-way choices with explanations («Как готовите») are stacked option rows: 15/600 title, 12.5px secondary explanation, a right-aligned value («7 дней / 1 день / 2 дня») that turns blue when selected. Segmented controls stay for two-way toggles only.
- **Offers (`.offer`):** a targeted ad block: 72px product image (radius md, white bg) + body `11/600 uppercase «Реклама» · 15px title · secondary text · «Промокод» with a dashed-border mono code that copies on click · blue CTA link with ExternalLink 13px · erid line`. Placed under «Где купить» on the recipe page, above the cart list and above the shopping list; one per placement. Hidden entirely while the global ads switch is off.
- **Admin «Реклама» tab:** a big `.switch--big` card (title + explanation) at the top, then «Предложения» (`AdminOffers`: `.alist` rows with flag, place badge, «регионов: N», until-date; `.ownform` with place segmented control and hint, `PhotoField` for the product image, region picker with `.chips` + `.suggest`, match datalist, date range, live `OfferCard` preview) and «Партнёрские магазины».
- **Focus on inputs:** `.form-control:focus` is a blue 1px border, no glow, radius unchanged (the global `:focus-visible` radius does not apply to fields).
- **Assistant «thinking» icon:** `Sparkles.twinkle` pulses scale 1→1.25→0.9 with a ±12° rock over 1.4s; it does not spin. Reduced motion: static.
...
- **Prep** — `.dish__prep--fridge` tertiary, `--freezer` blue, `--fresh` green; в строке заготовок снежинка синяя, холодильник `quaternary`.
...
- **Busy** — `.dish__swap.is-busy`: `spin` 720ms; `aria-busy="true"` на чеке/рецепте во время загрузки; искры помощника — `.twinkle` (мерцание, не вращение).
