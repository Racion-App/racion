...
export function CartSheet({ open, onClose, storeCode, storeName, country, items, onToast }: { open: boolean; onClose: () => void; storeCode: string; storeName: string; country: string; items: Item[]; onToast: (s: string) => void }) {
  const { t } = useT();
  const targets = cartTargets(storeCode, storeName, country);
  const [target, setTarget] = useState<CartTarget | null>(targets[0] ?? null);
  const copy = async () => {
      ...
      <ul className="cart__list">
        {items.map((i) => (
          <li key={i.id} className="cart__item">
            ...
            {target && (
              <a className="btn btn-soft btn-sm" href={searchURL(target, i.name)} target="_blank" rel="noopener" onClick={() => track("cart_open", { store: target.code })}>
